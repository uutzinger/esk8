...
# Control Left and Right Speed with Joystick.
# Urs Utzinger
# 2023
#####################################################################
# Joystick up increases speed, down decreases speed
# Joysitck left increases speed on right motor and decreases speed of left motor
# Button top: brings system to halt
# Button bottom: stears to center
# Button C: Turbo mode
# Button D: Battery display, Speed display, OFF
#
# Can adjust joystick sensitivty and speed increase/decrease
# Can decreasing speed faster than increasing speed
# Should work forward and backwards
#####################################################################
...
# if left_rigth is 0 speed_left is same as speed_right.
...
def handleEvent(event=None, tank=None, logger=None):
    #####################################################
    # Decode the gamepad events and call corresponding routines
    #####################################################
...
class TankDrive(object):
    ...
    def __init__(self):
        ...
        self.MAXLR         =   7   # max joystick value
        self.MINLR         =  -6   # min jpystick value
        self.SENS          =   1.5 # non linear joystick sensitivity
        self.BREAK         =   2   # chage speed faster when breaking
        self.MAX_SPEED     =  30   # max speed value
        self.MIN_SPEED     = -30
        self.THRESH_SPEED  =   0.1  # for stopping this amounts zero
        self.MAX_RATIO     =  1.
        self.MIN_RATIO     = -1.
        ...
        self.speed_right   =  0.
        self.eco           =  True
        self.stopping      =  False # Stopping mode, speed can only increase and steering straighten
        self.locked        =  False
        self.ratio_previous=  0.
    ...
    def update(self, REL_X=0, REL_Y=0):
            ...
            # Getting erroneous values of -123 in Y or X sometimes, therefore reject value if out of bounds

            if (REL_Y>=self.MINUPDOWN and REL_Y<=self.MAXUPDOWN) and (REL_X>=self.MINLR and REL_X<=self.MAXLR):
                if REL_Y < 0:
                    _up_down    = - (REL_Y/self.MINUPDOWN)**self.SENS # -1..1
                else: 
                    _up_down    =   (REL_Y/self.MAXUPDOWN)**self.SENS
                if REL_X < 0:
                    _left_right = - (REL_X/self.MINLR)**self.SENS     # -1..1
                else:
                    _left_right =   (REL_X/self.MAXLR)**self.SENS     # -1..1

                # Speed will be adjusted incrementally by joystick value
                # Speed is clamped

                # Are we accelerating or breaking?
                _speed_inc = self.SPEED_GAIN * _up_down
                if self.speed > 0:         # FORWARD MODE
                    if _speed_inc < 0:     # BREAKING
                        _speed_inc = _speed_inc * self.BREAK # break more than accelerate
                    else:                  # ACCELERATING 
                        if self.stopping:  # want to slow down
                            _speed_inc = - _speed_inc  # force to go slower by reversing increase
                ...
                self.ratio_previous =  self.ratio # keep prvious
                self.ratio  += _ratio_inc # update
                self.ratio = clamp(self.ratio, self.MIN_RATIO, self.MAX_RATIO) # clamp
    ...
    def stop(self):
        '''
        Want to slow down and straigthen
        Any joystick input will decreate the speed and straighten the vehcilce
        '''
